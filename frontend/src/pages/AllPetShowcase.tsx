...
export default function AllPetShowcase() {
  ...
  const searching = (searchValue: string) => {  // step 4: triggered by the typing of search word, even one input character will trigger the searching of the required information in the cloned petData
    if (searchValue.trim() === "") {
  ...
  useEffect(() => {   // step 6: after the change of search is confirmed (which means we confirm the value in the current search state is exactly the same as out text input), do the filtering actio by cloning and filtering the petData. The filtered item is then stored in petDataForSearch and rendered in UI
    let clonedPetData = [...petData]
    let filterPetData = clonedPetData.filter((eachPetData) => eachPetData.name.toLowerCase().slice(0, search!.length) === search  || eachPetData.name.toLowerCase().includes(search!) || eachPetData.name.toLowerCase()[0] === search![0] || eachPetData.age.toString().slice(0,search!.length) === search )
  ...
  if (data && data !== petData && !isSearching) {  // step 1: to ensure fetched data is 1) successfully collected and 2) without being the same as the petData value, 3) under the condition of not doing searching (which means no texts in the search bar)
  ...
  return (
    ...
    >
      <div className={styles.containerForAll}>
        ...
        {petDataForSearch?.map((pet) => <AnimalShow key={pet.pet_id} pet={pet} clickHandler={() => navigate(`/adoption/detail?id=${pet.pet_id}`)} />)}
