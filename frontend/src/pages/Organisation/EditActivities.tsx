...
import { FileButton, Group, LoadingOverlay, NativeSelect, NumberInput, Textarea } from "@mantine/core"; //https://ui.mantine.dev/category/inputs
...
export default function EditActivities() {
  ...
  const [fee, setFee] = useState<number | any>(0.0);
  ...
  const getOrganisationActivities = async () => {
    ...
    const data = await fetchJson<ActivityPreview[]>(
      `${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editActivities/getActivities/${organsationID}`,
      {
        method: "GET",
        headers: {
  ...
  const deleteActivity = async (activityToBeDeleted: ActivityPreview) => {
    ...
    const resp = await fetch(`${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editActivities/deleteActivities`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
        "Content-Type": "application/json",
  ...
  const handleSubmitForm = async () => {
    if (
      parseInt(activityStartTime.slice(0, 2)) > 23 ||
      parseInt(activityStartTime.slice(3, 5)) > 59 ||
      activityStartTime[0] === "_" ||
      activityStartTime[1] === "_" ||
      activityStartTime[3] === "_" ||
      activityStartTime[4] === "_" ||
      parseInt(activityEndTime.slice(0, 2)) > 23 ||
      parseInt(activityEndTime.slice(3, 5)) > 59 ||
      activityEndTime[0] === "_" ||
      activityEndTime[1] === "_" ||
      activityEndTime[3] === "_" ||
      activityEndTime[4] === "_" ||
    ...
    if (!file) {
      //stop the submission if no photo is submitted
      alert("請提供圖片");
      return;
    }
    ...
    let resp = await fetch(`${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editActivities/addActivities`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  return (
    <>
      ...
      <div className={styles.upperPart}>
        ...
        <div
          className={styles.chevronAndAdjustmntIcon}
          onClick={() => {
        ...
        <div className={styles.searchBarPart}></div>
        {/* <div className={styles.filter}><Filter className={styles.filterIcon} /></div> */}
      </div>

      <div className={styles.searchChance}>活動編輯</div>
      <div className={styles.tabList}>
        <Tabs defaultValue="基本資料" color="ocean">
          <Tabs.List className={styles.choices}>
            <Tabs.Tab className={styles.choiceFontSize} value="新增活動" onClick={() => click("addActivities")}>
              新增活動
            </Tabs.Tab>
            <Tabs.Tab className={styles.choiceFontSize} value="編輯活動" onClick={() => click("deleteActivities")}>
              編輯活動
            </Tabs.Tab>
          </Tabs.List>
        </Tabs>
          ...
          <div className={styles.formPart}>
            <form>
              <Input.Wrapper id="activitiyName" label="活動名稱" withAsterisk>
                <Input id="activitiyName" {...register("activitiyName", { required: true })} />
              </Input.Wrapper>

              <Textarea label="活動內容" withAsterisk id="activityDetails" {...register("activityDetails", { required: true })} />
              ...
              <Textarea label="參加要求" withAsterisk id="requirements" {...register("requirements", { required: true })} />

              <NativeSelect data={["九龍", "香港島", "新界"]} label="舉辦地區" radius="md" size="md" withAsterisk id="district" {...register("district", { required: true })} />

              <Textarea label="舉辦地址" withAsterisk id="address" {...register("address", { required: true })} />
              ...
              <div className={styles.uploadImagePart}>
                {" "}
                ...
                {file && <p>Picked image: {file.name}</p>}
              </div>
          ...
          <div>
            {activityToBeDisplay?.map((activity) => (
              <Activity
                ...
                activity={activity}
                clickHandler={() => navigate(`/activity/detail?id=${activity.activity_id}`)}
                displayDeleteButton={true}
                onRemove={() => deleteActivity(activity)}
                activityToBeDeleted={activity.activity}
              />
            ))}
          </div>
        )}
