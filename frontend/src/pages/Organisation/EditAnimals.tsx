...
export default function EditAnimals() {
  ...
  const [weight, setWeight] = useState<number | any>(0.0);
  ...
  const getOrganisationAnimals = async () => {
    const data = await fetchJson<PetPreview[]>(
      `${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editAnimals/getAnimals/${organsationID}`,
      {
        method: "GET",
        headers: {
  ...
  const deleteAnimal = async (animalToBeDeleted: PetPreview) => {
    ...
    const resp = await fetch(`${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editAnimals/deleteAnimals`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
        "Content-Type": "application/json",
  ...
  const handleSubmitForm = async () => {
    ...
    let resp = await fetch(`${process.env.REACT_APP_NODE_ENV === "production" ? process.env.REACT_APP_BACKEND_URL : "http://localhost:8080"}/editAnimals/addAnimals`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  useEffect(() => {
    animalToBeDisplay?.filter(
      (eachAnimalData) =>
        eachAnimalData.name.toLowerCase().slice(0, search!.length) === search || eachAnimalData.name.toLowerCase().includes(search!) || eachAnimalData.name.toLowerCase()[0] === search![0]
  ...
  return (
    <>
      ...
      <div className={styles.searchChance}>新增待領養的動物</div>
      <div className={styles.tabList}>
        <Tabs defaultValue="基本資料" color="ocean">
          <Tabs.List className={styles.choices}>
            <Tabs.Tab className={styles.choiceFontSize} value="新增動物" onClick={() => click("addAnimals")}>
              新增動物
            </Tabs.Tab>
            <Tabs.Tab className={styles.choiceFontSize} value="編輯動物" onClick={() => click("deleteAnimals")}>
              編輯動物
            </Tabs.Tab>
          </Tabs.List>
        </Tabs>
        ...
        {choice === ContentToBeDisplayed.addAnimals ? (
          <div className={styles.formPart}>
            <form>
              <Input.Wrapper id="animalName" label="動物名稱" withAsterisk>
                <Input id="animalName" {...register("animalName", { required: true })} />
              </Input.Wrapper>

              <NativeSelect data={["male (男)", "female (女)"]} label="性別" radius="md" size="md" withAsterisk id="gender" {...register("gender", { required: true })} />

              <Input.Wrapper id="age" label="年齡" withAsterisk>
                <Input id="age" {...register("age", { required: true })} placeholder="輸入格式為=> 月/年-月/年 months/years old" />
              </Input.Wrapper>
              ...
              <Input.Wrapper id="breed" label="品種" withAsterisk>
                <Input id="breed" {...register("breed", { required: true })} placeholder="如品種不詳請填上 => 不詳" />
              </Input.Wrapper>

              <Textarea label="備注：" withAsterisk id="remark" {...register("remark", { required: true })} placeholder="如無請填上=> 無" />
          ...
          <div>
            {animalToBeDisplay?.map((eachAnimal) => (
              <AnimalShow
                key={eachAnimal.pet_id}
                pet={eachAnimal}
                clickHandler={() => navigate(`/adoption/detail?id=${eachAnimal.pet_id}`)}
                displayDeleteButton={true}
                onRemove={() => deleteAnimal(eachAnimal)}
                animalToBeDeleted={eachAnimal.name}
              />
            ))}
          </div>
        )}
