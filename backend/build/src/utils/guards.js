        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!token) {
                        // console.log("byebye");
                        return [2 /*return*/, res.status(401).json({ msg: "Permission Denied" })];
                    }
                    payload = jwt_simple_1.default.decode(token, jwt_1.default.jwtSecret);
                    return [4 /*yield*/, routes_1.userService.verifyUser(payload.id, payload.role)];
                case 1:
                    user = _a.sent();
                    if (user) {
                        req.user = __assign(__assign({}, user), { role: payload.role });
